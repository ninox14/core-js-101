...
function retry(func, attempts) {
  ...
  return () => func(); // yes ofcourse this line is necessary
  // cancer linter rules REEEEEEEEEEEEEEEEEEEEEEEEEEEEE
}
...
function logger(func, logFunc) {
  return (...ar) => {
    logFunc(`${func.name}(${JSON.stringify(ar).slice(1, -1)}) starts`);
    const res = func(...ar);
    logFunc(`${func.name}(${JSON.stringify(ar).slice(1, -1)}) ends`);
    return res;
  };
}
